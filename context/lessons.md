...
2026-04-15: Two-way audio architecture invariants — (1) talk_url in config.yaml must point to go2rtc's TLS port (1985), NOT Flask's port; (2) buildTalkSocketUrl must mirror the talk_url protocol (https→wss) — changing it to ws: causes Mixed Content blocks on HTTPS pages; (3) go2rtc.yaml merge conflicts silently prevent TLS from loading, making wss:// fail — always verify `[api] tls listen addr=:1985` appears in `journalctl -u go2rtc` before debugging the JS; (4) if /talk/end fires ~2 seconds after /talk/start with no WebRTC activity in between, the WebSocket connection was blocked (Mixed Content or TLS mismatch), not a signaling issue.
...
2026-04-12: Behind Cloudflare Tunnel, `request.remote_addr` is always `127.0.0.1` — read the real client IP from the `CF-Connecting-IP` header instead; whitelist entries must be the public IP, not the LAN IP.
2026-04-12: All log.jsonl writers (append_log_entry, rewrite_log) and readers (read_log) must acquire fcntl.flock on a dedicated log.jsonl.lock sidecar file — missing the lock on any one writer allows silent data loss when the archiver's atomic rename races against an append.
2026-04-12: Use read_log(config) in the archiver rather than reading log.jsonl directly — the flock protocol must cover all readers too (LOCK_SH), not just writers, to prevent a rewrite from racing a read mid-rename.
2026-04-12: werkzeug.security.safe_join must gate both the JPEG check and the .age check in the /images route — checking only one leg leaves a path traversal window for the other.
2026-04-12: patch_log_entry always rewrites log.jsonl even on a miss — add a short-circuit before calling rewrite_log when the transform finds no matching row to avoid unnecessary SD card writes.
2026-04-12: Two-phase rsync delete (sync all → confirm exit 0 → delete local files) is safer than --remove-source-files, which deletes source files as each is transferred and leaves a partial state on network interruption.
2026-04-13: For tracked config changes, update `config.yaml.example` rather than inventing a local `config.yaml`; `_build_section` automatically wires new frozen dataclass fields with defaults, so Phase A audio config only needed the dataclass fields and example YAML keys added.
2026-04-14: After a worktree-based Codex implementation, always audit all changed files with `git status` before merging — Codex left monitor.py, test_monitor.py, and MOM_GUIDE.md uncommitted in the worktree, so they were missing from the branch.
2026-04-14: Any stream-name or device-name referenced in JS must be injected via a template data attribute (data-stream-name="{{ stream_name }}") — hardcoding `?src=grandma` in a constant violates the config-as-single-source-of-truth rule.
2026-04-14: Config feature-flag fields (e.g. chime_before_talk) must be consulted at the call site; a defined-but-ignored field is a maintenance hazard — wire it or remove it before merging.
